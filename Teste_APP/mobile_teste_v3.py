...
class AgendaScreen(Screen):
    ...
    def has_work(self, year, month, day):
        ...
        self.agenda_dados = carregar_agenda()
        return date_str in self.agenda_dados and len(self.agenda_dados[date_str]) > 0
    ...
    def show_info(self, year, month, day):
        date_str = f"{year}-{month:02d}-{day:02d}"
        trabalhos = self.agenda_dados.get(date_str, [])
        ...
        btn_modify.bind(on_release=lambda x: self.modify_work(date_str))
...
class ModifyWorkScreen(Screen):
    ...
    def prepare_for_modification(self, date_str):
        ...
        trabalhos = carregar_agenda().get(date_str, [])
        if trabalhos:
    ...
    def save_info(self, instance):
        ...
        if hora and descricao:
            ...
            if not found:
                ...
                agenda_dados[self.date_str].append({'hora': hora, 'descricao': descricao})
